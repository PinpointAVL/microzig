...
pub fn Usb(comptime f: anytype) type {
    return struct {
        ...
        pub fn task(debug: bool, rx_buffer: []u8) !usize {
            ...
            if (ints.BuffStatus) {
                ...
                while (iter.next(&iter)) |epb| {
                    ...
                    switch (epb.endpoint_address) {
                        ...
                        Endpoint.EP2_OUT_ADDR => {
                            std.log.info("Got RX data, {any}", .{epb.buffer});
                            bytes_received = epb.buffer.len;
                            std.mem.copyForwards(u8, rx_buffer[0..bytes_received], epb.buffer[0..bytes_received]);
                            f.usb_start_rx(
                                epb.endpoint_address,
                                64,
                            );
                        },
                        else => {
            ...
            if (S.configured and !S.started) {
                S.started = true;
                f.usb_start_rx(Endpoint.EP2_OUT_ADDR, 64);
            }
